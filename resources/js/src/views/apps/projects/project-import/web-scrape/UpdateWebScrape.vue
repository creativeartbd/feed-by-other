<template>
    ...
    <b-tabs v-else v-model="tabIndex" v-on:activate-tab="tabActivated">
        <b-tab active>
                        ...
                        <b-row>
                            ...
                            <b-col md="12" class="mb-2" v-if="is_csv_xml_import_visible">
                                ...
                                <validation-provider
                                    ...
                                    rules="required"
                                >
                                    <b-form-select v-model="mappingData.csv_xml_import" :options="csv_xml_import_list" :state="errors.length > 0 ? false : null" multiple></b-form-select>
                                    <small class="text-danger">{{ errors[0] }}</small>
                                </validation-provider>
                            </b-col>
                            <b-col md="12" class="mb-2" v-if="is_external_sitemap_visible">
                                ...
                                <b-button variant="outline-success" @click="addNewRow">
                                    +
                                </b-button>
                            </b-col>
        ...
        <b-tab :title-item-class="{ disabledTab: disableMappingTab === true }">
            <template v-slot:title>
                <feather-icon
                    icon="MenuIcon"
                    class="mr-50 my-icon"
                ...
                <b-row class="mt-2 border-top m-0 mb-1">
                    <b-col cols="12" class="pt-1 p-0">
                        <b-button
                            ...
                            >Back</b-button
                        >
                        <b-button variant="primary" type="submit" @click.prevent="validateSetting" class="mr-2">
                            Next
                        </b-button>
                        <b-button
                            variant="success"
                    ...
                    <b-col md="12">
                        <app-timeline>
                            <app-timeline-item
                                ...
                                :time="notification.created_at ? null : notification.updated_at"
                                variant="success"
                            />
                        </app-timeline>
                    </b-col>
...
export default {
    ...
    props: ["importlist", "id_feed", "rows", "islog"],
    data() {
        return {
            loading: true,
            btnLoading: false,
    ...
    created() {
        ...
        // Get the CSV / XML list first
        http.get(
            "projects/csv-xml-import/get/" + this.mappingData.user_id + "/" + this.mappingData.token
        )
            .then((response) => {
            ...
            .then((response) => {
                ...
                if (Object.keys(response.data).length !== 0) {
                    ...
                    mapping_data.forEach((item) => {
                        //console.log(item)
                        //***************************/
                        // Prepare Settings tab data
                        //***************************/
                        this.mappingData.mapping_name = item.feed_name;
                        this.mappingData.import_period = JSON.parse(item.params).import_period;
                        ...
                        this.mappingData.scrape_once = item.scrape_once ? true : null;
                        this.mappingData.scrape_method = JSON.parse(item.params).scrape_method;
                        this.mappingData.use_proxies = JSON.parse(item.params).use_proxies;
                        this.mappingData.safety = JSON.parse(item.safety_info);
                        this.mappingData.mapping = JSON.parse(item.scrape_rules);
                        this.mappingData.scrape_from = JSON.parse(item.params).scrape_from;
                        ...
                        if (this.mappingData.scrape_from === 1) {

                            this.is_csv_xml_import_visible = true;
                            this.is_regex_url_visible = false;
                            this.is_external_sitemap_visible = false;
                            this.is_from_specifc_pages = false;
                            this.mappingData.csv_xml_import = JSON.parse(
                                item.params
                            ).scrape_from_value;

                        } else if (this.mappingData.scrape_from === 3) {
                            ...
                            let params_parsed = JSON.parse(item.params);
                            let scrape_from_value = JSON.parse(params_parsed.scrape_from_value);

                            scrape_from_value.forEach((item, index) => {
                                this.mappingData.external_sitemap_url.push({
                                    url: item.url,
                                });
                            });

                        } else if (this.mappingData.scrape_from === 4) {

                            this.is_csv_xml_import_visible = false;
                            this.is_regex_url_visible = true;
                            this.is_external_sitemap_visible = false;
                            this.is_from_specifc_pages = false;
                            this.mappingData.regex_url = JSON.parse(item.params).scrape_from_value;

                        } else if ( this.mappingData.scrape_from === 5 ) {
                            ...
                            this.mappingData.from_specific_pages_input = scrape_from_value.from_specific_pages_input;
                            this.mappingData.from_specific_pages_option = scrape_from_value.from_specific_pages_option;

                            scrape_from_value.from_specific_page_url.forEach( ( item ) => {
                                this.mappingData.from_specific_page_url.push({
                                    url : item.url
                                });
                            });
                        ...
                        //*************************************/
                        // Prepare Settings tab data end here
                        //**************************************/

                        //***************************/
                        // Prepare Mapping tab data
                        //***************************/
                        this.mappingData.mapping = JSON.parse(item.scrape_rules);
                        //***********************************/
                        // Prepare Mapping tab data end here
                        //***********************************/
                    });
    ...
    methods: {
        handleRemoveMore(index) {
            ...
            this.mappingData.mapping[index - 1].rule_name =
                "Rule#" + this.mappingData.mapping.length;
        },
        handleAddMore() {
...
.vqb-group-heading .card-header,
.vqb-group-body .card-body {
    padding-left: 0;
}
